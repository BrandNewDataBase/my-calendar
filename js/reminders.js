...
import { state, persist, reloadFired, DAY_MS } from './store.js';
...
export function checkDue() {
  ...
  for (const o of occs) {
    for (const off of (o.ev.reminders || [])) {
      const dueAt = o.occStart.getTime() - off;
      if (now < dueAt || now >= dueAt + GRACE) continue;
      if (o.occStart.getTime() < now - 60000) continue; // 이미 시작한 일정은 제외
      const key = `${o.occId}:${off}`;
